...
public class UVNode implements LNode,P2PNode, Serializable,Comparable<UVNode> {
    ...
    private void processUpdateAddHTLC(final MsgUpdateAddHTLC msg) {
        ...
        if (!forwardingChannel.reservePending(this.getPubKey(),amt_forward)) {
            ...
            log("Reserved liquidity on "+forwardingChannel.getId()+ " to forward: "+amt_forward + " from "+this.getPubKey()+ " to "+getChannelPeer(forwardingChannel.getId()));
        }
   ...
   public void broadcastToPeers(String fromID, GossipMsg msg) {
    ...
    protected synchronized boolean pushSats(String channel_id, int amount) {
        ...
        if (new_node1_balance >= 0 && new_node2_balance >= 0) {
            ...
            log("pushSats: Insufficient funds in channel "+target_channel.getId()+" cannot push  "+amount+ " sats to "+peer);
            return false;
        }
        return true;
    }
